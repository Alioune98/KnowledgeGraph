...
public class IndexTree {
	...
	public final BinarySearchTree2<Entity1> index;
	private Entity1 tempent = new Entity1("temp");//Added by Matthew
    ...
    public IndexTree(String fileName) throws FileNotFoundException {
        ...
        while(scan.hasNextLine()){
            ...
            while((token = scan.findInLine("[\\p{L}']+")) != null){
                ...
                //Added by Matthew
                count++;
                int placement = count%3;
                addRecord(token, placement);
                
                //Added by Matthew
                
                //Commented Out to make work for entity
    ...
    public void addRecord(String w, int line){
    	/*	Commented Out by Matthew to change index Tree to work for entity
    	...
    	*/
    	...
    	//Added by Matthew
    	/*Entity1 entity = new Entity1 (w);
    	if(!index.contains(new Entity1(w))) {
    		index.add(new Entity1(w));
    	}else {
    		//else add next two data points into the entity's relations and dependents arrays
    		Entity1 temp = index.find(entity.getEntity());
    	...
    	if(line==1) {
    		...
    		if(line==2) {
    			tempent.addRelation(new Relation ("newrel"));
    			//change temp
    			//add relation for previous entity
    		}
    	}
    	//Added by Matthew
